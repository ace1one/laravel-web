...
@section('content')
    ...
    <!--main content-->
    <section class="content logs px-3">
        <div class="card ">
                        ...
                        <thead>
                            <tr>
                                @foreach ($headers as $key => $header)
                                    <th class="{{ $key == 'date' ? 'text-start' : 'text-center' }}">
                                        @if ($key == 'date')
                                ...
                                @foreach ($rows as $date => $row)
                                    <tr>
                                        @foreach ($row as $key => $value)
                                            <td class="{{ $key == 'date' ? 'text-start' : 'text-center' }}">
                                                @if ($key == 'date')
                                                ...
                                                @else
                                                    <a href="{{ URL::to('admin/log_viewers/logs', [$date, $key]) }}">
                                                        <span
                                                            class="level level-{{ $key }}">{{ $value }}</span>
                                                    </a>
                                                @endif
                                            </td>
                                        @endforeach
                                        <td class="text-end">
                                            ...
                                            <a href="{{ URL::to('admin/log_viewers/logs/' . $date . '/download') }}"
                                                class="">
                                                <i class="livicon" data-name="download" data-size="18" data-loop="true"
                                                    data-c="#00bc8c" data-hc="#00bc8c"></i>

                                            </a>
                                            <a href="#" data-bs-toggle="modal" class="delete"
                                                data-log-date="{{ $date }}">
                                                <i class="livicon" data-name="remove-alt" data-size="18" data-loop="true"
                                                    data-c="#f56954" data-hc="#f56954"></i>
                                            </a>
                                        </td>
                                    </tr>
                                @endforeach
