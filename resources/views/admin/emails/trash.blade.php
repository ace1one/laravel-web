                            ...
                            @foreach ($emails as $email)
                                <tr data-messageid="{{ $email->id }}" class="unread">
                                    <td class="inbox-small-cells width7">
                                        <div class="checker">
                                            <span>
                                                <input type="checkbox" name="trash_checkbox" value="{{ $email->id }}"
                                                    class="mail-checkbox">
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="view-message d-none d-sm-inline width22">
                                        <a href="{{ URL::to('emails/' . $email->id . '/trash') }}">
                                            @if ($email->senderName->provider_id && $email->senderName->pic)
                                                <img src="{!! $email->senderName->pic !!}" alt="img" width="35"
                                                    height="35"
                                                    class="img-circle img-responsive img_height float-start" />
                                            @elseif($email->senderName->pic)
                                                <img src="{!! url('/') . '/uploads/users/' . $email->senderName->pic !!}" alt="img" width="35"
                                                    height="35"
                                                    class="img-circle img-responsive img_height float-start" />
                                            @else
                                                <img src="{{ Gravatar::src($email->senderName->email) }}" width="35"
                                                    height="35" alt="User Image"
                                                    class="img-circle img_height img-responsive float-start" />
                                            @endif
                                            {{ $email->senderName->first_name }} {{ $email->senderName->last_name }}
                                        </a>
                                    </td>
                                    <td class="view-message width40">
                                        <a
                                            href="{{ URL::to('emails/' . $email->id . '/trash') }}">{{ $email->subject }}</a>
                                    </td>

                                    <td class="view-message text-end width23">
                                        <a
                                            href="{{ URL::to('emails/' . $email->id . '/trash') }}">{{ $email->created_at->diffForHumans() }}</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
