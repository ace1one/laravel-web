    ...
    <div class="container">
        <div class="row">
            <div class="box animation flipInX">
                ...
                <form action="{{ route('forgot-password-confirm', compact(['userId', 'passwordResetCode'])) }}"
                    class="omb_loginForm pwd_validation" autocomplete="off" method="POST">
                    {!! Form::token() !!}
                    <label class="visually-hidden"> New Password</label>
                    <input type="password" class="form-control" name="password" placeholder="New Password">
                    <span class="form-text">{{ $errors->first('password', ':message') }}</span>
                    <label class="visually-hidden">Confirm Password</label>
                    <input type="password" class="form-control mt-15" name="password_confirm"
                        placeholder="Confirm New Password">
                    <span class="form-text">{{ $errors->first('password_confirm', ':message') }}</span>
                    <div class="d-grid">
                        <input type="submit" class="btn btn-primary mt-2" value="Submit to Reset Password">
                    </div>
