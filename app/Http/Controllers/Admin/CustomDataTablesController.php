...
class CustomDataTablesController extends Controller
{
    ...
    public function index()
    {
        ...
        $countries = Datatable::select('country')->distinct()->pluck('country');

        $max_count = Datatable::all()->count();   // We can pass max count for slider
        $max_id = Datatable::pluck('id')->max();
        $min_id = Datatable::pluck('id')->min();
        return view('admin.examples.custom_datatables', compact('professions', 'countries', 'max_count', 'max_id', 'min_id'));
    }
    ...
    {
        if ($request->countrySelect !== null && $request->countrySelect !== 'all') {
            $tables = Datatable::where('country', $request->countrySelect)->get(['id', 'firstname', 'lastname', 'email', 'country', 'age']);
        } else {
            $tables = Datatable::get(['id', 'firstname', 'lastname', 'email', 'country', 'age']);
        }
        return DataTables::of($tables)
            ->make(true);
    }
